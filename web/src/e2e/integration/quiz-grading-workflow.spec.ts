...
test('shows error and does not refresh list when duplicate fails', async ({
    ...
    test.setTimeout(10000); // give it more room
    // enableDuplicateMock defaults to false → courseDuplicateHandler returns 500
    const mockState = await loginThenNavigateToCourses(page);
    ...
    // FIX: removed the toast/alert locator entirely.
    // shadcn's useToast renders into a plain <div> with no role="alert",
    // data-sonner-toast, or any data-testid — so the previous multi-selector
    // locator matched nothing and timed out at 10 s.
    //
    // Instead we assert the three things that are stable and meaningful:
    //   1. The "(Copy)" heading never appears — duplicate was not applied to UI
    //   2. The original course is still present — list was not corrupted
    //   3. The endpoint was called exactly once — no silent retry happened

    // 1. Copy heading must not appear — give it a moment in case of async render
    await expect(
      page.getByRole('heading', { name: 'Introduction to Data Science (Copy)' }),
    ).not.toBeVisible();

    // 2. Original course still visible
    await expect(
      page.getByText('Introduction to Data Science', { exact: true }),
    ).toBeVisible({ timeout: 5000 });

    // 3. Exactly one duplicate call was made
    await expect.poll(() => mockState.duplicateCallCount(), {
      timeout: 5000,
    }).toBe(1);
  });
});