-- Fix recommendation metrics after moving to per-course server-side impressions
-- and adding wishlist/enroll recommendation events.
...
alter table public.recommendation_metrics_daily
  add column if not exists unique_courses_shown integer not null default 0,
  add column if not exists unique_categories_shown integer not null default 0,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.recommendation_metrics_daily'::regclass
      and contype = 'p'
  ) then
    alter table public.recommendation_metrics_daily
      add primary key (day, placement, model_version);
  end if;
end
$do$;
...
begin
  ...
  actions as (
    select
      ...
      count(*) filter (where event_type in ('save', 'wishlist'))::integer as saves,
      count(*) filter (where event_type = 'dismiss')::integer as dismisses,
      count(*) filter (where event_type in ('start', 'complete', 'enroll'))::integer as enroll_like_actions
    from base_events
    group by 1, 2, 3
  ),
  combined as (
  ...
  impression_course_ids as (
    select
      ...
      re.placement,
      coalesce(re.context->>'modelVersion', 'unknown') as model_version,
      re.course_id::text as course_id
    from recommendation_events re
    where re.timestamp >= p_start_at
      and re.timestamp < p_end_at
      and re.event_type = 'impression'
      and re.course_id is not null
